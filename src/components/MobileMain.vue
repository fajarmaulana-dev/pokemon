...
<template>
    <div class="flex transition duration-500 [&>*]:min-w-full [&>*]:h-[calc(100vh-60px)] [&>*]:z-[1] [&>*]:overflow-x-hidden
        ...
        <section>
            <PokemonCards id="beranda" v-model:text="models.search['beranda']" name="home-mobile-filter"
                ...
                :load="filterLoad['beranda']" :data="filteredPokemon['beranda']"
                v-model:on-bottom="models.onBottom['beranda']" @open-card="val => emit('openCard', val)"
                :error="error['beranda']" :refresh-load="refreshLoad['beranda']" @refresh="emit('refresh')"
                @confirm="({ index }) => emit('cardConfirm', { index, name: 'beranda' })"
            ...
            <div class="flex w-full h-full transition duration-500 [&>*]:min-w-full [&>*]:z-[1]"
                ...
                <div class="overflow-hidden">
                    <PokemonCards id="wilayah" v-model:text="models.search['wilayah']" name="region-filter"
                        ...
                        v-model:confirm="models.confirmData['wilayah'].confirm"
                        @heart="val => emit('handleFavourite', val)" v-model:on-bottom="models.onBottom['wilayah']"
                        @open-card="val => emit('openCard', val)"
                        @confirm="({ index }) => emit('cardConfirm', { index, name: 'wilayah' })"
                        :no-detail="noDetail['wilayah']" :refresh-load="refreshLoad['wilayah']"
                        @refresh="emit('refresh')" :data="filteredPokemon['wilayah']"
                        v-model:loadmore="models.filter['wilayah'].next" :error-more="errorMore['wilayah']"
        ...
        <section>
            <PokemonCards id="favorit" v-model:text="models.search['favorit']" name="favourite-mobile-filter"
                ...
                :load="filterLoad['favorit']" :disable-filter="disableFilter"
                @open-filter="val => emit('openFilter', val)" :data="filteredPokemon['favorit']"
                @open-card="val => emit('openCard', val)" :refresh-load="refreshLoad['favorit']"
                @refresh="emit('refresh')" :no-detail="noDetail['favorit']"
    ...
    <aside class="fixed h-screen w-full top-0 right-0 transition duration-500 bg-white p-4"
        ...
        <Preference v-if="pageHandler == 1" :model-value="prefer"
            @update:model-value="val => emit('update:prefer', val)" :err-net="profileError.prefer" :temp="preferTemp"
            @save="val => emit('savePrefer', val)" @back="emit('backProfile')" :load="profileLoad.prefer"
            @file="val => emit('profileImage', val)" />
        <Password v-if="pageHandler == 2" @back="emit('backProfile')" :err-net="profileError.password"
        ...
        <Help v-if="pageHandler == 3" @back="emit('backProfile')" />
        <Term v-if="pageHandler == 4" @back="emit('backProfile')" />
        <Privacy v-if="pageHandler == 5" @back="emit('backProfile')" />
    </aside>
    <!-- Pokedex -->
