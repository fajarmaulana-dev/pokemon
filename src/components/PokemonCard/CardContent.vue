...
<template>
    <div ref="ripple" @click="animateRipple($event); emit('open')" class="w-[calc(100%-8rem)] xx:w-[calc(100%-8rem)] p-4 text-slate-800
    cursor-pointer overflow-hidden relative [&>*:not(section)]:relative [&>*:not(section)]:z-[10]">
        ...
        <h4
            class="font-bold !text-base xx:!text-lg xs:!text-xl mb-3 capitalize whitespace-nowrap overflow-hidden text-ellipsis">
            {{ data.name }}</h4>
